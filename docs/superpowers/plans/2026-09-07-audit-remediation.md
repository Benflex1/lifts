...
- [ ] Replace `App.tsx`'s unconditional ready state after failure with loading/ready/error states and a Retry action. No main UI or providers may issue storage reads before initialization succeeds.
- [ ] Rerun the integration test, `npm test`, and `npx tsc --noEmit`. On native, upgrade an existing fixture installation and verify no data disappears. Gate: initialization failures are visible; transaction rollback and migration idempotence are demonstrated.
